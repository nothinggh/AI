import pandas as pd # type: ignore

from src.db import fetch_dataframe, fetch_all, fetch_one

def table_counts():
    return fetch_dataframe("""
        SELECT 'BOM' AS table_name, COUNT(*) AS row_count FROM BOM
        UNION ALL
        SELECT 'MFP' AS table_name, COUNT(*) AS row_count FROM MFP
        UNION ALL
        SELECT 'INSP' AS table_name, COUNT(*) AS row_count FROM INSP
        UNION ALL
        SELECT 'YDP' AS table_name, COUNT(*) AS row_count FROM YDP
        """)

##########################################################################

def table_list():
    return fetch_dataframe("""
        SELECT name AS table_name
        FROM sqlite_master
        WHERE type = 'table'
        ORDER BY name
        """)

##########################################################################

def bom_summary_by_ship(keyword: str = ""):
    """호선(ship_no)별 BOM 총수량, 총중량, 총금액을 집계합니다."""
    where = ["ship_no IS NOT NULL AND ship_no != ''"]
    params = []

    if keyword:
        where.append("ship_no LIKE ?")
        params.append(f"%{keyword}%")

    where_clause = " AND ".join(where)

    return fetch_dataframe(
        f"""
        SELECT
            ship_no,
            COALESCE(SUM(quantity), 0) AS total_quantity,
            ROUND(COALESCE(SUM(weight), 0), 2) AS total_weight,
            COALESCE(SUM(price * quantity), 0) AS total_price
        FROM BOM
        WHERE {where_clause}
        GROUP BY ship_no
        ORDER BY ship_no
        """,
        tuple(params),
    )

##########################################################################

def item_type_counts():
    """품목 타입(item_type)별 수량, 중량, 금액 집계"""
    return fetch_dataframe("""
        SELECT 
            item_type,
            COALESCE(SUM(quantity), 0) AS total_quantity,
            ROUND(COALESCE(SUM(weight), 0), 2) AS total_weight,
            COALESCE(SUM(price * quantity), 0) AS total_price
        FROM BOM
        GROUP BY item_type
        ORDER BY item_type
        """)

##########################################################################

def bom_by_ship_no(ship_no: str):
    return fetch_dataframe(
        """
        SELECT 
            id,
            user_id,
            ship_no,
            item_type,
            material,
            size,
            quantity,
            weight,
            price,
            order_date,
            request_note
        FROM BOM
        WHERE ship_no = ?
        ORDER BY id
        """,
        (ship_no,),
    )

##########################################################################

def get_ship_summary_df():
    query = """
    SELECT 
        ship_no AS ship_no,
        manager AS manufacturer,
        COUNT(DISTINCT unit_no) AS drawing_count,
        COUNT(CASE WHEN issue IS NOT NULL AND TRIM(issue) <> '' THEN 1 END) AS issue_count,
        SUM(actual_hours) AS total_hours,
        SUM(headcount) AS total_headcount,
        SUM(actual_hours * headcount) AS total_man_hours
    FROM YDP
    GROUP BY ship_no, manager;
    """
    df = fetch_dataframe(query)
    df.columns = ['호선 번호', '제작업체', '도면 갯수', '이슈 건수', '토탈 실투입 시간', '작업자 수', '총 공수(M/H)']
    return df
df_result = get_ship_summary_df()
print(df_result)

##########################################################################

def load_drawing_status():
    """DB에서 데이터를 조회하여 호선별 집계 데이터를 생성합니다."""
    # 1. DB 데이터 조회
    query = """
        SELECT 
            ship_no,
            dwg_type,
            vendor,
            status,
            issue_type,
            worker
        FROM drawings
    """
    df = fetch_dataframe(query)

    # 2. 조회 대상 전체 호선 목록
    target_hulls = ["SN101", "SN201", "SN301"]

    summary_list = []

    for hull in target_hulls:
        df_hull = df[df["ship_no"] == hull] if not df.empty else pd.DataFrame()

        # 도면 등록이 1건도 없는 호선 처리
        if df_hull.empty:
            summary_list.append(
                {
                    "ship_no": hull,
                    "dwg_type": "-",
                    "vendor": "-",
                    "도면행수": 0,
                    "status": "작업 전",
                    "완료건수": 0,
                    "이슈건수": 0,
                    "worker": "-",
                }
            )
        else:
            total_count = len(df_hull)

            # status가 '완료'인 항목 수 카운트
            completed_count = (
                df_hull["status"].astype(str).str.strip() == "완료"
            ).sum()

            # issue_type이 '없음'이 아닌 항목 수 카운트 (이슈 건수)
            issue_count = (
                df_hull["issue_type"].astype(str).str.strip() != "없음"
            ).sum()

            # 전체가 완료되었으면 '완료', 그 외는 '진행 중'
            current_status = "완료" if completed_count == total_count else "진행 중"

            # 작업자 목록 (중복 제거 후 쉼표 연결)
            workers = ", ".join(
                df_hull["worker"].dropna().astype(str).str.strip().unique()
            )

            summary_list.append(
                {
                    "ship_no": hull,
                    "dwg_type": df_hull["dwg_type"].iloc[0],
                    "vendor": df_hull["vendor"].iloc[0],
                    "도면행수": total_count,
                    "status": current_status,
                    "완료건수": completed_count,
                    "이슈건수": issue_count,
                    "worker": workers,
                }
            )

    return pd.DataFrame(summary_list)

##########################################################################
